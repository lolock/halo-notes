...
译：克劳德项目是 [claude.ai](https://claude.ai/) 上最未被充分利用的功能。大多数人根本不使用它们。他们打开一个新的聊天室，输入一个问题，获得答案，然后关闭选项卡。每节课都从零开始。
...
译：我每天打开的第一件事。取代 45 分钟的手动扫描 ⏱ 每天早上 7 点使用 这个已在本系列文章第 1 篇中详细介绍 - 但它在这里赢得了榜首，因为它具有最高的每日回报。在我接触其他内容之前，这个项目为我提供了 3 分钟的简报，涵盖了我需要了解的所有内容。
...
译：写的帖子、话题和文章实际上听起来像我。 ⏱ 每周使用 5-7 次 使用 AI 制作内容的最大问题是它听起来像 AI。通用的。光滑的。令人难忘。这个项目解决了这个问题，因为它上传了我的作品——不仅仅是关于我的风格的说明，还有**我帖子的实际示例**。
...
译：我上传了 30 条表现最好的推文和 5 篇令我引以为傲的文章。系统提示告诉它匹配那个声音，而不是发明一个。
...
**中文：✓ 我的产出从每周 2 篇文章增加到 7 篇 - 花费的创意精力相同**
...
译：深入研究并记住我已经探索过的一切。 ⏱ 每周使用 3-4 次 这与仅在聊天中询问 Claude 问题之间的区别：该项目会记住每个研究会话。我会随时上传调查结果、笔记和摘要。随着时间的推移，它构建了一个可搜索的复合知识体系。

When I come back to a topic months later, I don't start from scratch - I pick up where I left off.

译：当我几个月后回到某个主题时，我不会从头开始 - 我会从上次停下的地方继续。
...
译：我读过、思考过和保存的所有内容都可以搜索和连接。 ⏱ 每周使用 2-3 次 大多数笔记系统都会失败，因为它们是只写的。你保存了一些东西，然后就再也找不到它们了。这个项目将克劳德变成了一个活跃的知识助手——它不仅仅是存储，它还**连接和综合**。
...
译：每周我都会将笔记、重点和想法转储到正在运行的文本文件中并重新上传。然后，克劳德就可以了解我几个月的想法，并可以揭示我从未手动建立的联系。
...
译：在 10 分钟内清除我积压的电子邮件。 ⏱ 每天使用 我每天将收件箱粘贴到该项目中一次。它返回时，每封电子邮件都已贴上标签，紧急邮件已写好草稿回复，其余邮件已归档。过去需要 30 分钟精神能量的事情现在只需不到 10 分钟。
...
**中文：✓ 电子邮件焦虑消失 - 我确切地知道什么需要我，什么不需要我**
...
译：知道我的堆栈。不再需要每次都从头开始解释上下文。每周使用 4-5 次 在常规聊天中使用 Claude 编写代码的令人沮丧的事情是，在讨论实际问题之前，您需要花 5 分钟解释您的堆栈、约定和约束。这个项目完全消除了这一点。
...
译：我上传了我的技术堆栈、命名约定以及我正在从事的主要项目的自述文件。现在，每个代码问题都会得到一个与我正在构建的内容实际上兼容的答案。
...
**中文：✓ 消除了 80% 的复制粘贴修复循环，而这些循环会破坏我的心流状态**
...
译：当我需要诚实的思考而不是舒适的答案时，我会使用它。 ⏱ 每周使用 2-3 次 这是最难解释的项目，因为它是最个人化的。这不是一个任务自动化项目——而是一个有思想的合作伙伴。我提出了一些重大问题：商业决策、创意方向、我坚持的事情。
...
译：它与在常规聊天中询问克劳德有何不同：它知道我的**目标、限制、决策历史以及我倾向于如何思考**。它可以指出我的模式和偏见，因为它们已被记录下来。
...
**中文：✓ 我拥有的最好的思考工具 - 我提供的背景越多，它就会变得越清晰**

## How to Build All 7 in a Weekend

## 中文：如何在一个周末内完成所有 7 个任务

Don't try to build all 7 at once. Here's the order that makes sense:

译：不要尝试一次构建所有 7 个。这是有意义的顺序：
...
译：- **第 1 天早上（30 分钟）：** 构建项目 01（早间简报）- 最高的即时投资回报率
- **第 1 天下午（45 分钟）：** 构建项目 02（内容引擎）- 上传您的写作样本
- **第 2 天早上（30 分钟）：** 构建项目 05（收件箱清零）- 当天使用并观察节省的时间
- **第 2 天下午（30 分钟）：** 构建项目 07（战略家）- 诚实地写下您的目标和偏见文件
- **第 2 周：** 根据需要添加项目 03、04、06 - 它们的设置工作量较大，但具有长期价值
...
> 中文：> **复合效应：** 随着您更新文件，每个项目都会随着时间的推移而变得更好。 90 天后，这 7 个项目将比大多数同事更了解你的工作。设置成本为一个周末。此后每天都会返回。
...
> 中文：> **大多数人跳过的一件事：** 上下文文件。系统提示很容易复制。但是文件——你的写作样本、你的笔记、你的偏见、你的堆栈——这些使得每个项目都属于你，而不仅仅是一个花哨的聊天窗口。将 80% 的设置时间花在文件上。
