...
自托管环境（self-hosted environments）现已进入公开测试版，让你可以在自己的基础设施上运行 Claude Code 会话。无论是从网页、手机、桌面端，还是通过 routine 启动会话，会话都将在你的网络内部运行，紧邻你的内部服务、工具链和安全控制，而不是运行在 Anthropic 托管的基础设施上。

对大多数企业，我们强烈推荐托管方案（hosted offering），因为它运维简单，无需自行运行或维护基础设施。自托管环境面向的是这样一些团队：其网络、工具或合规要求需要将 agent 执行保留在自己掌控的基础设施上。如果选择这条路线，请安排工程人员负责环境搭建和后续维护。
...
- **网络访问：** 会话在你的网络内部运行，可以访问内部服务、数据库和镜像仓库（registries），无需将它们暴露到公网。
- **可定制性：** 在环境中预先安装编译器、SDK 和内部 CLI，让每次会话一开始就具备构建所需条件。
- **合规性：** 源代码和构建产物保留在你控制的基础设施上。

「自托管环境让我们能够在保持安全与运营控制的同时，将 Claude Code 集成到现有的开发工作流中。这意味着 Claude 可以生成 PR、帮助修复 CI 问题、响应开发者工作流事件，而且算力可以根据需求扩展。Claude 理解我们的代码库，非常适合我们工程团队的构建方式。」——George Jacob，高级工程经理（Faire）
...
仓库检出（checkouts）、构建产物、密钥，以及会话创建或修改的任何文件，都保留在你自行配置的基础设施上。

但对话本身——包括提示词、响应以及工具结果（其中可能包含 Claude 读取的代码）——会发送给 Anthropic 进行推理；会话记录（transcript）也会被保存，以便你从任何终端继续会话。
...
- **按需模式（On-demand）：** 一个 orchestrator 监控排队中的会话，在会话到达时启动 runner，并在工作完成后停止它们，使容量随需求变化。

Runner 可以同时服务多个会话，但每个会话都在自己独立的 checkout 中运行，因此不同开发者和账户之间的工作相互隔离。来自所有支持终端的会话都会路由到同一个环境，所以你只需配置一次，团队无论从何处启动会话都能使用它。

注意：自托管环境不同于[远程控制（Remote Control）](https://code.claude.com/docs/en/remote-control)。远程控制让开发者可以从手机或浏览器继续运行在自己机器上的会话；使用 Remote Control 的会话会在那台机器停止运行该会话时结束，并且与运行 `claude` 的用户绑定。而自托管环境是在你的平台团队运营的共享基础设施上运行会话，任何用户都可以使用。
...
自托管环境现已以公开测试版形式向采用 Claude Team 和 Enterprise 计划的组织开放。该功能默认关闭，也不适用于使用 ZDR 的组织。
