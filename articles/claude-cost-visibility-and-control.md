...
本文将介绍 IT 管理员如何利用现有的管控功能来查看和管理 Claude 的成本，并分享一些决定资金投向的最佳实践。
...
第一个问题的答案取决于你的业务和需求——没有供应商能替你衡量。第二个问题可以通过让模型与任务相匹配来解决。让较便宜的模型承担复杂的推理任务，往往会使最终成果更加昂贵，因为它会在重试上消耗 Token，还需要更多人工修正。而让前沿模型处理基础的文档处理，则是在为任务根本用不到的能力付费。
...
对于上述任何模型，effort 控制都可以调高或调低模型在解决问题时的“思考”程度；advisor 工具则让较小模型只有在遇到瓶颈时才去咨询前沿模型。许多组织会同时使用多个模型，通常是在同一个项目中。例如，一家保险公司可能让前沿模型帮助理赔员评估复杂的商业理赔，同时用 Haiku 为输入其中的文档打标签并进行分流。
...
你能使用的控制取决于 Claude 是以面向员工的产品形态运行，还是作为应用背后的 API。前者把控制权交给管理员，后者则交给在其上构建应用的工程师，而大多数大型客户两者都会使用。
...
我们通常建议按顺序逐步启用这些功能，因为在看到一个月的真实用量之前，很难设定一个合理的上限。

- **访问控制（Access gating）**：让管理员决定哪些用户组和自定义角色可以使用 Claude Code、Claude Cowork 等产品，而不是一次性全部开通。先从单个团队开始，观察效果后再逐部门扩展。
- **模型控制（Model controls）**：在两个层面生效。Entitlements（权限）决定团队可以访问哪些模型，而 defaults（默认值）决定新会话默认从哪个模型开始。管理员可以授权承担最困难工作的团队使用最强大的模型，并将其他所有人默认设为 Sonnet。
- **硬性支出上限（Hard spend caps）**：为使用量设置上限。当你了解了整个组织、单个用户或某个用户组的基线后即可设置；按组设置时，组内每个成员都会获得该限额。上限会立即生效。
...
- **分析聊天（Analytics chat）**：让管理员用自然语言询问用量情况。可以直接问“我们这个月支出最多的是谁？”或“哪个团队本季度用量增长最快？”，无需拉取完整报告。
...
Claude Console 为在 Claude Platform 上构建的组织和开发者提供控制功能。Workspaces（工作区）按产品、团队或环境分隔 API 用量，并在你的成本与用量报告中拥有独立条目。Claude Platform 上实用的成本调节手段包括：
...
将这些功能组合使用，通常可以在任何人调整预算之前，就大幅降低生产工作负载的成本。
...
成本管控功能已在 Claude Enterprise 中提供。要查看方案和定价，请访问 claude.com/pricing。企业组织可以直接使用 Claude Enterprise 产品开始使用。开发者可以在 docs.claude.com 找到 Workspaces、缓存和批处理的文档。
