...
先说明一下：我从未在 Linear 工作过，也从未看过他们的代码。我分享的一切都来自我的个人经验、对他们应用的研究、阅读他们的博客文章，或观看他们的会议演讲。我只是热爱构建 Web 应用，并且从 Linear beta 发布起就在使用它。另外，本文的头图来自 Meg Wayne 的一段视频，她为 Linear 做的作品非常出色。原文链接：<https://performance.dev/how-is-linear-so-fast-a-technical-breakdown>
...
## 浏览器中的数据库 / Database in the browser
...
## 本地更新与后台同步 / Local updates and background synchronization

<!-- bilingual:section -->

<!-- lang:zh -->

第一行 `issue.title = "Faster app launch"` 会更新内存中的数据存储（Linear 这里是 MobX observable）。第二行 `issue.save();` 会把一个事务加入队列，由他们的同步引擎批量处理并刷新到服务器。关键在于：UI 会基于本地内存中的更新同步重新渲染。没有 spinner，因为没有任何需要等待的东西；数据会在后台同步。这就是把浏览器视为每个用户的数据库所带来的魔力。
...
## 无延迟创建 Issue / Linear's issue creation no spinners or delays
...
## 不依赖网络延迟的响应式界面 / UI responsiveness independent of network latency
...
## Linear 技术栈一瞥 / A peek into Linear's stack
...
最让我印象深刻的是，他们决定坚持客户端渲染。CSR 经常因初始加载缓慢而受到批评，但只要架构和设计得当，也可以让用户感觉应用几乎瞬间可用。

我也非常喜欢它带来的简单性。让应用完全保持客户端化，会形成更清晰的心智模型，并消除许多服务端渲染应用带来的复杂性。你不必总是思考自己当前是在服务器端还是客户端，也不必考虑 `window` 对象是否可访问、缓存头是否设置正确。简单自有其美，而被迫接受的约束同样可以带来美感。

那么，Linear 是如何让客户端渲染的应用感觉瞬间可用的？
...
## 让首次加载感觉瞬间完成 / Making the first load feel instant

<!-- bilingual:section -->

<!-- lang:zh -->

我非常关注首次加载，Linear 显然也是如此。尤其对于生产力工具来说，在真正开始工作之前需要等待多久，是最重要的细节之一。没有人想等一个新标签页加载好几秒。

首先，你必须理解是什么让初始加载变慢。对于客户端应用，你需要先请求 `index.html`，然后由它请求所有 JavaScript 和 CSS；这些资源接着运行某种身份验证逻辑，最后再发起一些 API 请求来展示应用。
...
## Linear 的打包器历程：Parcel、Rollup、Vite、Rolldown / Linear's bundler arc: Parcel, Rollup, Vite, Rolldown

<!-- bilingual:section -->

<!-- lang:zh -->

让应用感觉瞬间可用的第一步，早在运行时之前就已经发生了。它始于构建时。记住，网络是瓶颈，因此尽可能少地发送 JavaScript 和 CSS，对快速加载至关重要。
...
即便做了所有这些优化，Linear 仍然发送了相当多的代码：大约 21 MB 的压缩后 JavaScript。区别在于，这些代码被激进地拆分成数百个路由级 chunk，并按需获取。
...
这里的经验不是应该选择哪个打包器，而是：放弃旧浏览器、采用原生 ESM，以及疯狂进行代码分割有多么重要。每一步看起来都很小，但叠加起来，它们让 Linear 首次加载所需的 JavaScript 大约减少了一半，并让构建时间缩短了一个数量级。

所以，让加载时间变得近乎瞬时的第一个秘诀，就是减少为了向用户渲染出内容所需的 JavaScript 和 CSS。
...
## 初始加载后的预加载 / Preloading after initial load
...
但等等，把 bundle 分割成数百个 chunk 会带来一个新问题。每个 chunk 都会导入其他 chunk，而浏览器在解析入口脚本之前并不知道这些依赖是什么。如果没有额外帮助，加载时间线就会变成瀑布：获取入口、解析它、获取它的导入、解析那些导入，再获取它们的导入。每一层都会增加一次网络往返，而这是你必须不惜一切代价避免的。
...
## 预加载与缓存策略 / Preloading and caching strategy

<!-- bilingual:section -->

<!-- lang:zh -->

每个 preload 上的 `crossorigin` 属性都和入口脚本上的 `crossorigin` 匹配，因此浏览器会复用已缓存的请求，而不会把 preload 和 import 当作两个不同的资源来处理。这和字体 preload 的技巧一样，只是应用到了关键路径上的每个 chunk。

冷启动加载时间线从顺序瀑布变成了单个并行批次。网络仍然在工作，只是一次性全部完成。这个技巧的美妙之处在于：当用户第一次到达登录页面时，你就能在后台完成所有这些工作。几秒钟后，完整应用已经存入缓存，并可以瞬间提供服务。
...
## 用 Service Worker 获得更快速度和离线能力 / The service worker for even more speed and offline capabilities
...
## Vendor bundle 的组成 / Vendor bundle composition
...
## 加载巨大的字体文件 / Loading massive font files

<!-- bilingual:section -->

<!-- lang:zh -->

字体加载是很多应用容易做错的细节之一。失败模式非常明显：文字消失半秒、真实字体替换进来时发生布局偏移，以及因为 preload 不匹配导致资源被请求两次。Linear 的设置避免了这三种情况：
...
可变字体用单个 woff2 覆盖完整的 100–900 字重轴，消除了按字重发起多个请求的需要。`font-display: swap` 会立即渲染 fallback 字体栈，并在 Inter 加载完成后替换。容易被忽略的技巧是：在 preload 标签上使用 `crossorigin="anonymous"`。如果没有它，浏览器会先预加载字体，稍后 CSS 引用该字体时又会再次获取，因为两次请求的 CORS 模式不同。preload 上的 `crossorigin` 能让浏览器复用已经缓存的那次请求。

这些看起来都很简单，但我总是惊讶于有多少应用会错误地加载字体。Linear 是一个很好的例子：他们认真思考每个细节，确保字体加载尽可能快速且准确。
...
## 内联应用外壳 / Inlined app shell

<!-- bilingual:section -->

<!-- lang:zh -->

另一个让首次加载感觉快速的关键技术，是在 `<head/>` 中内联刚好足够的 CSS，用来绘制加载状态，而不需要获取外部样式表。请记住，网络是瓶颈，也是你为了让应用感觉快速而始终需要对抗的因素。在这个例子中，Linear 通过内联展示应用外壳所需的关键 CSS，消除了一个网络请求。
...
除了 CSS，还有一批对加载初始体验至关重要的内联 JavaScript。
...
在任何 bundle 被解析之前，`index.html` 中的 JavaScript 会读取 `localStorage.splashScreenConfig`，把任何 `sessionStorage` 覆盖合并进去，并将用户记住的 shell token 直接应用到 `document.documentElement.style`：侧边栏背景、基础颜色、边框颜色、侧边栏宽度和 agent 工具栏高度。它会检测 color-scheme 偏好和 Electron 上下文，还会检查 `localStorage.ApplicationStore` 是否存在；如果不存在，就添加 `logged-out` 类，将 shell 切换到认证布局。

等第一个 JavaScript bundle 从网络返回时，加载屏已经根据用户是否登录，完成了正确的主题、尺寸和位置设置。这会给用户一种感觉：他们在 URL 栏按下回车的瞬间，应用就已经准备好了。没有比在初始 `index.html` 响应中直接发送初始应用外壳更快的办法。
...
## 先渲染，再认证 / Render first, authenticate second

<!-- bilingual:section -->

<!-- lang:zh -->

身份验证是另一个大多数应用会放弃性能预算的环节。传统流程是：获取 HTML、加载 bundle、验证 session、获取用户、获取 workspace，然后渲染。用户要等一到三秒才能看到任何内容。

Linear 对待认证的方式，就像对待 mutation 一样：假设 happy path 成立，并在后台验证。这可能是我最喜欢的架构部分之一，因为它让 Linear 几乎可以在加载时立即渲染完整体验。

大多数 CRUD 应用会把真正的 session 放在 HttpOnly cookie 中，然后再添加第二个 JavaScript 可读 cookie，或者发起 `/me` 请求，让前端在启动期间判断用户是否已登录。Linear 做得更简单。它没有维护一套并行的认证信号，内联启动脚本只是检查 `localStorage.ApplicationStore` 是否存在：
...
如果它存在，说明用户以前在这个浏览器中使用过 Linear，也就是说，他们的 workspace 已经位于 IndexedDB 中。这又回到了我们在第一节讲到的内容：数据库存在于浏览器中。如果它不存在，那本来也没有东西可渲染，所以 shell 会切换到登出布局，并由登录流程接管。
...
Linear 的初始流程不是“你是否有有效 session”，而是“我们是否有东西可以展示给你”。他们真正的 session token 位于 cookie 中。bundle 从不试图对此耍聪明。它只是渲染已有内容，并让下一个请求（WebSocket 握手、同步增量或任何 HTTP 调用）在 session 过期时以 401 失败。当这种情况发生时，客户端会重定向到登录页。
...
这可能是我最喜欢 Linear 的细节之一，我希望更多应用也能这样表现。对认证而言，假设 happy path 成立；如果不成立，再 fallback。如果有数据可以展示，就展示它！并利用浏览器的数据存储立即渲染。
...
## 1. 数据已经在那里了 / 1. The data is already there

<!-- bilingual:section -->

<!-- lang:zh -->

应用启动时，它不会从服务器获取 workspace，而是从 IndexedDB 水合到内存中的 MobX 对象池；UI 的每个查询都会先访问这个对象池。不存在“正在加载 issues”的状态，因为 issues 已经在用户机器上了。

随着规模增长，他们还在同步引擎中采用了类似 JavaScript bundle 的基本思想，对数据进行分块。并不是所有东西都一次性获取：最重的两张表 `Issue` 和 `Comment` 会按需懒水合。这是数据层面的代码分割，也是引擎能够扩展的原因：启动成本取决于 workspace 的结构，而不是 workspace 的大小。一个拥有 10,000 个 issue 的 workspace，启动速度大约和一个拥有 100 个 issue 的 workspace 一样快。

点击进入一个项目，issues 已经在那里。按负责人过滤，索引已经构建好。没有东西需要获取，因为没有东西缺失。它要么已经从浏览器中立即加载，要么会很快从一个按需切分的懒加载块中加载。
...
## 2. Mutations 不等待网络 / 2. Mutations don't wait for the network
...
用户永远不需要等待才看到自己的更改。重试、回滚以及跨刷新持久化，全都在后台完成。如果服务器拒绝，observable 会回退，并出现短暂闪烁；但实践中这几乎不会发生，因为大多数无效 mutation 在事务创建前就已经被捕获了。
...
## 3. 一个增量，一个单元格 / 3. One delta, one cell
...
因为 Linear 中每个模型的每个属性本身都是 observable，而每个读取它的组件都包裹在 `observer()` 中，MobX 确切知道哪些组件依赖哪些字段。一个更新某个 issue 的一个字段的变化，只会重新渲染读取该字段的组件：不是父列表，不是侧边栏，而是一个单元格。50 个 issue 的更新就是 50 个单元格重新渲染，而不是一次列表重新渲染。这就是为什么在十个人同时编辑内容时，一个繁忙的 workspace 仍能保持流畅：接收更新的成本随“变化了什么”扩展，而不是随“屏幕上有什么”扩展。

我曾构建过实时应用，流式传入股票数据和基本面数据；对单个组件进行原子更新，是让应用感觉高性能的关键。你要尽可能避免级联更新，而 Linear 正是这么做的。
...
## 为什么这三者必须配合 / Why the three fit together
...
Linear 速度的另一个基石，是他们把键盘整合为导航和完成工作的主要工具。每个常用操作都有快捷键。命令面板只需一个按键即可打开。右键菜单是自定义构建的。这些都不是偶然，而是从第一天起就经过深思熟虑的设计决策。
...
## 每个操作都有快捷键 / Every action has a shortcut
...
听创始人谈 Linear 早期时，很明显快捷键从一开始就是基础能力。同步引擎的一部分设计目标，就是让任何操作都可以在任何时候执行。感觉这种设计和工程的组合，持续支撑着每个功能。

如果你浏览他们的 UI，会注意到快捷键随处可见。最频繁使用的操作通常是单字符，因为它们使用得最多。此外，每个操作也都可以用鼠标完成，以免让新手感到疏离。
...
## 命令面板始终只差一个按键 / The command palette is always one keystroke away

<!-- bilingual:section -->

<!-- lang:zh -->

`⌘ k` 会打开命令面板，用户几乎可以搜索 Linear 中的任何操作：issues、projects、labels、状态变更、导航、创建 issue、设置和主题切换。命令面板极快，因为它搜索的是本地 MobX 对象池，而不是服务器。记住，要避免网络。
...
到目前为止的所有努力，仍然可能被糟糕的动画毁掉。团队花费巨大精力让应用的每个部分都很快：初始加载、更新、数据库查询，所有这些。他们削减毫秒级耗时，让用户永远不必等待。然后，在最后一步，有人给某个元素加了一个 500ms 的高度动画。
...
## 你应该动画化的属性只有少数几个 / There are only a handful of properties you should animate

<!-- bilingual:section -->

<!-- lang:zh -->

浏览器中属性变化有三个层级，其成本随着它在渲染管线中所处的位置升高而增加。合成属性（`transform`、`opacity`）会把工作交给 GPU，并独立于主线程运行。触发绘制的属性（`color`、`background-color`、`border-color`、`fill`）跳过布局，但仍然要重绘像素。触发布局的属性（`width`、`height`、`top`、`left`、`margin`、`padding`）会迫使浏览器重新计算页面上后续每个元素的位置。永远不要动画化这些属性。我是说，永远不要。
...
`margin-left` 版本会在整个 200ms 的过渡期间，每一帧都重新计算悬停行下方每一行的布局。在较长的 issue 列表中，这正是丝滑流畅与卡顿之间的差别。
...
如果你检查 Linear 应用中每一个被动画化的属性，会发现它们只保留在少数几项上，主要是那些合成属性（`transform` 和 `opacity`），有时也包括 `background-color` 和 `border-color`。
...
## 知道什么时候该克制 / Know when to hold back

<!-- bilingual:section -->

<!-- lang:zh -->

在我看来，几乎和“只动画化合成属性”同样重要的是，知道什么时候完全不要使用动画。动画很容易让人上头，但在一个每天都要使用的工具中，那些你会喜欢出现在营销网站上的动画，可能会开始碍事。哪怕只是一个很小的 hover 延迟，只要出现在错误的位置，也会变成用户注意到的问题。

Linear 在这方面大多数地方都做得很好。命令面板是我会认为太慢的一个，不过这些年我已经变成了一个挑剔的老头。
...
他们的许多动画之所以有效，是因为动画引用了自身的来源。状态 popover 从状态 pill 中缩放出来，agent 面板从它的开关处滑入。这种运动在空间上发挥了解释作用，告诉用户新元素从哪里来，而不是作为装饰从虚无中淡入。
...
## 保持时长短而干脆 / Keep durations short and snappy
...
大多数设计系统的默认时长都比应有的更长。Material 的标准时长是 200ms，iOS 的 spring 更接近 350ms。将默认过渡设得更短，是让应用感觉更快的最简单方法之一，而 Linear 的默认值明显低于行业常态。

Linear 更进一步，采用了进入和退出不对称的时序。Hover 高亮、popover 和 agent 面板在你召唤它们时会立即出现，然后在你关闭它们时用 150ms 淡出。Agent 窗口会立即出现，但像 macOS 一样淡出。
...
## Linear 是如何这么快的 / How Linear is so fast

<!-- bilingual:section -->

<!-- lang:zh -->

还有太多细节可以展开，而它们共同让 Linear 感觉快速。现实是，没有任何单一因素能让应用具备高性能；这是数百个正确决策累积的结果。
...
我喜欢 Linear 方法的一点是，其中大部分都很简单。没有 Next，没有 Tanstack，也没有花哨的框架。他们很早就决定了什么架构最能服务用户，并且一直忠于这一点。结果就是一个客户端渲染应用，比服务端渲染应用更快（而且没有那些复杂性）！
...
大致形态是这样的：服务器是同步目标，而不是事实来源。数据库位于浏览器中。Mutations 先在本地应用，并在后台协调。首次加载发送更少的代码，但拆成更多小块；service worker 会在用户仍停留在登录页时预缓存其余部分。认证基于状态先做假设，稍后验证。同步引擎从 IndexedDB 水合到逐属性的 MobX observable 中，因此，50 个 issue 的更新会带来 50 个单元格重新渲染，而不是一次列表重新渲染。输入模型是键盘优先的。每个常用操作都有快捷键，并配有全局命令面板。动画保持在 GPU 上，持续时间低于 100ms 的因果感知阈值，并且永远不对会触发布局的属性做动画。
...
难点不在实现本身，而在于多年如一日地投入工艺：当代码库成熟、扩展，并不断遇到新的约束时，仍然坚持这些原则。
