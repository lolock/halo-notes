...
## 从 Claude Code 到 Slack / From Claude Code to Slack

<!-- bilingual:section -->

<!-- lang:zh -->

在[上一篇文章](https://claude.com/blog/how-anthropic-enables-self-service-data-analytics-with-claude)中，我们介绍了如何通过三项核心工件，让 Claude 以约 95% 的准确率回答数据分析问题：
...
本文讨论 Anthropic 的数据团队如何借助 [Claude Tag](https://claude.com/product/tag)（公开测试版），把这套基础应用到公司其他同事的工作场景中。Claude Tag 是我们在 Slack 中的数据分析智能体的基础：任何人都可以向它提出与数据相关的问题，并获得由分析师所使用的同一套受治理定义支撑的答案。
...
让智能体变得准确，与把它部署到非分析师也能使用的地方，其实是两件截然不同的事。我们不会重复上一篇文章中关于准确性的建议，因为它们在这里依然适用。相反，我们将分享过去一年中关于如何在 Slack 中部署数据分析智能体的五条最重要经验，以及你应该如何思考分发、权限、数据新鲜度和可观测性。
...
你可以用[技能](https://claude.com/blog/steering-claude-code-skills-hooks-rules-subagents-and-more)教会 Claude 按照你的风格和要求完成任务。技能是一个 Markdown 文件，包含自然语言指令，以及 Claude 在需要时可以引用的文件。

我们做出的最重要架构决策，是把技能文件当作持续刷新的在线内容来维护，而不是一次性交付后就置之不理。数据模型一天之内可能变动多次：某个列被重命名、某个指标定义被修正，或某张表被废弃。每一次这类变化都需要在相对短的时间内落到技能文件中。如果 Claude 读到的还是上周二那份技能文件，它就会满怀信心地给出上周二的错误答案。

这种倾向尤其有害，因为数据消费者已经完全脱离了判断回答准确性所需的上下文。他们面前没有带趋势线或相关指标的仪表盘来帮助自己做“直觉检验”。在 Slack 里，他们可能只收到一两个数据点；如果这不是他们经常查看的数据，就很可能接受那个自信满满的错误答案。
...
最初，我们部署基于 Claude Tag 的数据分析智能体时，本能的想法是创建一个“知识技能”，教 Claude 该用哪些表、我们的语义层如何组织，然后就此收工。我们很快发现，这种做法能得到正确的数字，却止步于有用的洞察。

大多数数据消费者倾向于提出开放且模糊的问题，比如“是什么导致了这次下滑？”“你能预测月底会落在什么水平吗？”或者“把这份数据做成漏斗图看看”。要回答这些问题，智能体不仅需要知道数据在哪里，还需要知道分析师会如何处理它。

因此，除了知识技能之外，我们还为 Claude Tag 挂载了额外的分析技能或运行手册技能，包括：

- **预测**：何时以及如何拟合简单趋势、季节性假设，以及何时因为序列过短或噪声过大而拒绝回答。
- **群组与留存分析**：标准群组定义、上报给领导层的留存曲线模板，以及会令朴素实现出错的各类陷阱（如左删失、幸存者偏差）。
- **漏斗分析**：关键产品漏斗的规范阶段定义，从而让“用户在新手引导中流失在哪里？”这类问题在不同回答中保持一致。
- **图表**：可视化规范，例如什么类型的问题使用哪种图表、配色方案，以及何时表格比图形更清晰。
- **分析写作**：如何组织一条结论（先给 TL;DR，再给数字、机制和注意事项），以及根据置信度高低采用何种程度的谨慎措辞。
...
即便是知识技能与运行手册技能的组合，也不总能回答一个问题。当有人问“为什么周二注册量下降了？”时，答案往往不在数据模型里，而是分散在 Slack 讨论串、事故追踪系统、发布说明和文档中。

为了弥补这些缺口，我们把 Claude Tag 接入内部知识索引——它收录了全公司的文档、讨论和事件。当智能体发现某个指标异动时，可以在索引中检索同期上下文：当天早上打开的事故、被翻转的功能开关、某人在频道里分享的竞品公告。
...
Claude Tag 以服务账号而非提问者本人的身份查询数据仓库。虽然这是正确的设计（你总不希望每个 Slack 用户都需要直接的数据仓库凭据），但[任何能 @ 到这个机器人的用户都拥有机器人的数据访问权限](https://claude.com/blog/agent-identity-access-model)。这里没有按用户区分的行级安全：服务账号能读什么，频道里的任何人都可以问。

我们从五个方面来应对这一问题（建议认真对待，因为这件事容易做错且难以回退）：

1. **将服务账号的范围限定为仅受治理数据。** 在 Anthropic，Claude Tag 的服务账号可以读取语义层的输出表以及为这些表供数的精选数据集市。它不能读取原始事件流、暂存模式或个人沙箱中的任何内容。如果某个问题需要该边界之外的数据，智能体会如实说明，而不是猜测。这也是正确的用户体验，因为受治理层之外的数据尚未经过验证。
2. **在列级别对 PII 进行分类，并拒绝向服务账号授予访问许可。** 受治理数据并不自动等于 PII 安全数据（例如，精选表仍可能带有电子邮件地址）。我们维护了一个带列级血缘的数据目录，因此每一列的来源和下游流向都是已知的。当新列出现时，Claude 会扫描它们并标出疑似 PII 供人工审查。随后由人工在列的元数据中应用分类，血缘关系会将该标签传播到派生表。由于 Claude Tag 的服务账号没有任何 PII 访问许可，数据仓库的列级访问控制会让所有 PII 列对智能体不可见。它可以查询这张表，但敏感列根本读不到。
3. **在技能文件中记录连接路径。** 数据仓库技能中有一个专门章节，说明智能体如何连接（通过 CLI、直接 API 还是 MCP 服务器），以及每条路径的认证机制具体如何运作。这个平淡无奇的功能让我们能够区分“干净的失败”（“我无法从这个界面访问数据仓库，原因如下”）和“令人困惑的失败”（查询悄悄跑在错误的项目上，或认证提示被转发到了不该去的地方）。当连接机制写进技能文件后，智能体就能解释自己的限制。
...
5. **为每次查询打标签。** 对于每次数据仓库查询，Claude Tag 都会携带标识使用界面、对话和请求用户（在 Slack 提供该信息的情况下）的标签。这不会在查询时强制任何策略，但能提供成本归属和审计追踪（事后可以查出是谁问了那个扫描了 4 TB 数据的问题）。

我们的总体定位是：Slack 中的数据分析智能体是受治理数据仓库的一个共享只读副本，我们也尽量按这个定位来界定它的范围。
...
- 加载了哪些技能文件、版本是什么；
- 用户是否用 👍/👎 回应，或回复了更正；
- 它访问的表上是否有未解决的数据质量警告。我们还会在回答页脚中展示数据质量警告，让“数据可能过期”的提示出现在数字旁边，而不是隐而不见。

这些遥测数据支撑两个视图。一个追踪采用率：按界面和领域统计，有多少比例的智能体查询经由受治理层完成，而不是临时 SQL。另一个追踪正确性：按领域统计 👎 反应和更正的比率。这是在两次评估运行之间衡量准确率的在线代理指标。

事实证明，采用率是我们追踪的最具可操作性的指标。当某个领域的采用率下降时，几乎总是意味着某个技能文件已经过时，或出现了语义层尚未覆盖的新类型问题。
...
例如，一位数据团队成员问 Claude，为什么收入仪表盘的加载时间比平时慢了几分钟。Claude 发现查询结果没有被缓存，而且一个 bug 正在拖慢结果到达页面的速度。Claude 通知了仪表盘负责人，对方决定立即修复缓存问题，同时把那个 bug 作为另一项工作单独处理。负责人随后问还有哪些仪表盘变慢了，结果发现几十个仪表盘都受到同一个缓存错误的影响。Claude 写好了缓存修复代码，数据团队成员审查后，所有受影响的仪表盘在一个小时内就恢复了满负荷运行。

*说明：以上为虚构重现的 Claude Tag 对话，事故细节、人名和工具均为虚构。*

这些讨论串是公开的，这带来多方面的好处。旁观的同事能自己了解到上下文（出了什么问题、为什么、怎么修好的），而无需任何人专门写总结。更重要的是，他们不必一直当被动读者：任何掌握有用信息的人都可以加入进来贡献，就像上面例子中的团队成员所做的那样。因此，请把智能体放在共享频道里，并让工作在讨论串中进行，而不是私聊，因为讨论串可以充当可追溯、可供复核的历史记录。
...
大量数据工作都是重复性的：管道健康检查、KPI 监控等等。你可以请 [Claude 创建循环](https://www.youtube.com/watch?v=SlGRN8jh2RI)，按计划或在出现异常变化时处理周期性任务。我们已实现的一些数据场景包括：

- **主动播报**：Claude 在每周站会前提供摘要：上周有哪些变动、与前一周相比如何、有什么值得注意的地方。
- **测试监控**：在监控发布或实验时，Claude 每天提供多次播报。在最近的一次实验中，它注意到实验中途设置发生了变化，帮助我们及早发现并修复。
- **可观测性**：其他循环监控我们的管道和仪表盘。如果管道失败，Claude 会开始调查、起草修复方案并联系当值人员。如果某个 KPI 出现异常变动，Claude 会给出可能的解释——节假日效应？上游数据变化？——并在任何人打开仪表盘之前逐一核查。
- **分流**：另一个循环跟踪我们的数据问题频道。对每个新问题，它都会做出判断：直接回答、展开深入调查，还是请人类介入。等到数据团队成员查看时，大部分工作已经完成。
...
你可以允许 Claude 在选择的任何频道中更加主动：它在一旁阅读，需要时主动出手相助。在过去一个月里，在我们其中一个数据频道中，Claude Tag 回答了人们发布的超过 75% 的问题，通常在一两分钟内完成，即使没有人 @ 它。

例如，一位 Anthropic 团队成员在公开频道中询问某个仪表盘是否包含一个新的用量类别。90 秒内，Claude 就回答了数据如何定义，确认新分类确实缺失，提出修复方案，并起草了一个 PR。数据科学家审查并批准后，Claude 合并了 PR 并刷新了仪表盘。
...
- **再定分发。** 选择挂载仓库或基于 MCP 的技能，并端到端验证新鲜度：改动一个技能文件，确认 Claude Tag 在你的 SLA 时限内感知到变化。
- **从第一天就埋点。** 你无法给一个月前的对话补上遥测。从第一个问题起就记录结构化事件。
- **尽量接入知识索引。** 数据仓库回答“是什么”；内部文档和事故信息流回答“为什么”。一旦数据通路稳定，就尽快把它们接入。
- **最后添加分析技能。** 先创建数据访问技能，然后让真实问题告诉你，同事实际需要哪些分析技能（预测、群组、漏斗）。
