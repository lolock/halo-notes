...
从今天开始，Claude Managed Agents 可以按计划运行，并安全访问 CLI 工具及其他需要认证的服务。这两项功能现已在 Claude Platform 上进入 public beta。
...
## 按计划运行 Agent / Run agents on a schedule

<!-- bilingual:section -->

<!-- lang:zh -->

Agent 现在可以按计划运行，自动完成例行工作。[Scheduled deployment](https://platform.claude.com/docs/en/managed-agents/scheduled-deployments) 会为 Agent 配置 cron 计划。每当计划触发时，Agent 都会启动一个新会话并完成任务；你无需自行构建或托管调度器。
...
## 已有团队的应用 / How teams are using it
...
- [Rakuten](https://claude.com/customers/rakuten-qa) 使用 scheduled deployments 按周或按月分析电子表格数据，并生成报告和演示文稿。团队还会监控生产日志和指标，让产品经理无需创建 dashboard 就能了解应用的运行状况。
...
- [Ando](https://ando.so) 使用 scheduled deployments 让招聘和销售团队的工作持续推进。Agent 会自主监控各个渠道中提出的后续步骤，在到期时跟进，并发送会议提醒。
...
## 在 Vault 中保存环境变量，为 CLI 和其他工具提供认证 / Store environment variables in vaults to authenticate CLIs and other tools

<!-- bilingual:section -->

<!-- lang:zh -->

Agent 通过直接 API 调用、CLI 和 MCP [连接外部系统](https://claude.com/blog/building-agents-that-reach-production-systems-with-mcp)。现在，Anthropic 正在扩展 [vaults](https://platform.claude.com/docs/en/managed-agents/vaults)，使其支持环境变量，让 CLI 和其他工具可以发起经过认证的请求。CLI 让 Agent 能够通过 shell 直接驱动现有命令行工具，因此是一种快速、轻量的集成路径。你只需将 API key 与环境变量名及其可访问的域名一起注册，安装在 Agent 沙盒中的 CLI 就可以使用它发起经过认证的 API 调用。

Agent 永远看不到你的 key，因为沙盒中只保存一个占位符。真正的 key 会在网络边界附加，而且只会附加到发往你允许的域名的请求上，因此只会流向你批准的位置。要更换 key，只需在 vault 中更新；正在运行的会话会在下一次调用时获取新值。大多数会在 HTTP 请求中发送 key 的 CLI 都可以用这种方式工作，包括 [Browserbase](https://docs.browserbase.com/integrations/anthropic/managed-agents/quickstart)、[KERNEL](https://www.kernel.sh/docs/integrations/claude-managed-agents)、Notion、Ramp 和 Sentry CLI。Browserbase 和 KERNEL 还首次为 Managed Agents 带来了浏览器能力，使 Agent 能够与使用其他工具一样浏览并操作网页。
...
## 安全连接工具与自动化工作流 / Secure tool access and automated workflows
...
- Notion 使用 vault 中的环境变量，将 Notion CLI 与 MCP 工具一同部署，为其 agent 增加文件上传能力，同时确保 API token 始终不会交给模型。

- Browserbase 使用通过 vault 完成认证的 browse CLI，构建了公开的 browser skills 目录。一个定时部署会定期验证该目录，确保其保持准确。

- Milana 使用 vault 中的环境变量，将其 AI product engineer 安全连接到客户的代码库。该 agent 会自动发现并修复 bug，同时让大规模数据分析比以往运行得更快。
...
## 客户引述 / Customer quotes

<!-- bilingual:section -->

<!-- lang:zh -->

> “Vault 中的环境变量让我们能够安全推出 Notion CLI，满足安全团队的严格准则，确保敏感的 API token 永远不会交给 agent。CLI 是 MCP 工具的补充，使 Claude Managed Agents 具备文件上传能力。”
>
> — Quan Nguyen, Public API Lead

> “Rakuten 的多个团队使用定时部署，按周或按月分析电子表格中的数据，并生成报告或演示文稿。我们的高级用户将其用于生产日志和指标，因此产品经理无需创建分析 dashboard，就能了解其应用的健康状况。”
>
> — Yusuke Kaji, General Manager of AI for Business

> “我们大多数用户更愿意使用更少的 agent，而不是更多的 agent。借助定时部署，他们可以将更多能力整合到一个自主 agent 中。例如，一个 agent 可以关注多个销售和招聘流程，向合适的人员确认最新进展，并推动后续步骤。”
>
> — Sara Du, Founder

> “我们基于 Claude Managed Agents 构建了一个跨账户的 agentic search 系统，让销售团队可以询问诸如今天应该联系哪些账户之类的问题。由于客户希望这些答案定期刷新，我们用定时部署取代了自行构建的调度基础设施，这大幅简化了我们的技术栈，也改善了产品迭代周期。”

> “借助 Claude Managed Agents，我们让 agent 基于客户的实际代码库开展工作。我们发现它易于使用，而且几乎立刻就能获得高质量结果。关键突破来自 vault 中的环境变量：它让我们的 agent 能够通过 CLI 调用私有 API，同时不暴露凭证。现在，大规模数据分析的速度显著提升，而且我们可以通过结果确保每次输出的质量。”

> “Vault 中的环境变量让我们的工程团队能够把两个重要的计算原语结合起来：agent 和 browser。在 Browserbase，我们将 Claude Managed Agents 与 browse CLI 结合使用，生成公开的 browser skills 目录，帮助 agent 浏览网页；定时部署则会对这个公开目录进行周期性验证。”
...
你可以阅读我们的文档，进一步了解相关信息，也可以访问 Claude Console，部署你的第一个 agent。
